...
public final class GuiManager {
    ...
    public void openMy(Player p, ViewContext ctx) {
        ...
        List<Listing> mine = service.listMine(p.getUniqueId());

        int perPage = 45;
        int maxPage = Math.max(0, (mine.size() - 1) / perPage);
        if (ctx.myPage < 0) ctx.myPage = 0;
        if (ctx.myPage > maxPage) ctx.myPage = maxPage;

        decorateMy(inv, ctx, maxPage);
        fillMy(inv, ctx, mine);
    ...
    private void decorateMain(Inventory inv, ViewContext ctx, int maxPage) {
        ...
        boolean hasPrev = ctx.page > 0;
        boolean hasNext = ctx.page < maxPage;

        inv.setItem(52, hasPrev
                ? button(Material.ARROW, "&f< Пред.", List.of("&7Предыдущая страница"))
                : button(Material.GRAY_STAINED_GLASS_PANE, "&7< Пред.", List.of("&7Нет предыдущей страницы")));

        inv.setItem(53, hasNext
                ? button(Material.ARROW, "&fСлед. >", List.of("&7Следующая страница"))
                : button(Material.GRAY_STAINED_GLASS_PANE, "&7След. >", List.of("&7Нет следующей страницы")));
    }

    private void decorateMy(Inventory inv, ViewContext ctx, int maxPage) {
        inv.setItem(49, button(Material.ARROW, "&fНазад", List.of("&7Вернуться в аукцион.")));

        boolean hasPrev = ctx.myPage > 0;
        boolean hasNext = ctx.myPage < maxPage;
    ...
    private void fillMy(Inventory inv, ViewContext ctx, List<Listing> mine) {
        int perPage = 45;
        int start = ctx.myPage * perPage;
        int end = Math.min(mine.size(), start + perPage);

        for (int i = 0; i < perPage; i++) inv.setItem(i, null);

        long now = System.currentTimeMillis();
        if (start >= mine.size()) return;
    ...
    public static ItemStack tag(ItemStack it, String id) {
        ItemMeta meta = it.getItemMeta();
        if (meta != null) {
            meta.getPersistentDataContainer().set(Keys.LISTING_ID, PersistentDataType.STRING, id);
            it.setItemMeta(meta);
        }
        return it;
    }
    ...
    public static String listingIdFromItem(ItemStack it) {
        ...
        return it.getItemMeta().getPersistentDataContainer().get(Keys.LISTING_ID, PersistentDataType.STRING);
    }
