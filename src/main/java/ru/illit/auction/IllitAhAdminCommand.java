...
public final class IllitAhAdminCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!sender.hasPermission("illit.ah.admin")) {
            sender.sendMessage(U.c("&7[&f&lILLIT AH&7] &cНет прав."));
            return true;
        }
        if (args.length >= 2 && args[0].equalsIgnoreCase("ah") && args[1].equalsIgnoreCase("reload")) {
            plugin.reloadConfig();
            plugin.service().reload();
            sender.sendMessage(U.c("&7[&f&lILLIT AH&7] &aПерезагружено."));
            return true;
        }
        sender.sendMessage(U.c("&7[&f&lILLIT AH&7] &7Команды: &f/illit ah reload"));
        return true;
    }
}
